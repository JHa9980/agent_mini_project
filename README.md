...
## Architecture
<img width="280" height="2100" alt="architecture" src="https://github.com/user-attachments/assets/abc8e76c-d19e-48d2-a396-edac443eed7d" />
...
> **참고:** `config.py`가 모듈을 로드할 때 프로젝트 루트의 `.env`를 자동으로 읽어 LangSmith 추적 설정을 활성화합니다. 다른 경로에서 실행하더라도 `.env`가 루트에 존재하기만 하면 됩니다.
...
- `evidence_pool`: 세그먼트별 수집 증거(요약, 출처, 근거 신뢰 지수, 정량 신호)
...
수집된 텍스트 정보는 정량적 분석을 위해 다음과 같은 Signal과 Metric으로 변환됩니다.
...
### 근거 신뢰 지수 (Evidence Reliability Score) 규칙

근거 신뢰 지수는 LLM의 주관적 판단이 아닌, 명확한 규칙에 따라 계산되어 객관성과 일관성을 확보합니다.
...
정확한 분석을 위해 데이터 품질을 자동으로 진단하고, 부족한 부분은 `InfoCollector`에게 재수집을 요청하는 피드백 루프가 동작합니다.

- **`coverage-gap`**: 근거 신뢰 지수 0.6 이상인 증거가 없는 세그먼트. → 해당 세그먼트 재검색
- **`low-confidence`**: 세그먼트의 평균 근거 신뢰 지수가 0.5 미만. → 지수가 높은 출처 위주로 재검색
- **`low-volume`**: 세그먼트별 증거 개수가 2개 미만. → 증거 개수 확보를 위해 재검색
